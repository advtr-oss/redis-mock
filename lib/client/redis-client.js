 ...
 * but the command is handled by the function passed here
 *
 * May also add an addCommand method, so that any methods added here
 * are not automatically added to the prototype unless called again
 * like the issue mentions
 * */
RedisClient.prototype.addMockCommand = function (command, callback) {
  // Could just return here??
  // or do we ignore this and
  // just overwrite
  ...
  this.commands[command] = function () {
    // Probably don't need this but heigh-ho, snow white didn't need
    // 7 dwarfs but she was better with them
    const args = parseArguments(arguments);
    const cb = args.pop();
    return callback(self, args, cb);
  };
};

/**
 * I can see why this would help, the user can define the commands above in the test
 * suite but have the commands 'added' in the main code somewhere
 * */
RedisClient.prototype.addCommand = function (command) {
  ...
  if (!this[command]) {
    ...
    this[command.toUpperCase()] = this[command] = this.commands[command];

    // Alias special function names (e.g. JSON.SET becomes JSON_SET and json_set)
    if (commandName !== command) {
      this[commandName.toUpperCase()] = this[commandName] = this.commands[command];
    }
  }
  ...
  // // saves writing extra lines
  // // json.get, JSON.GET, json_get, JSON_GET
  // [command, command.toUpperCase(), commandName, commandName.toUpperCase()].forEach((cmd) => {
  //   this[cmd] = this.commands[command];
  //   multi.Multi.prototype[cmd] = function (...args) {
